...
class DataCleaner:
    ...
    def fix_missing_values(
        ...
        if strategy == "Delete":
            ...
            self.train = pd.DataFrame(
                data=imputer.transform(self.train), columns=self.variables
            )
            self.test = pd.DataFrame(
                data=imputer.transform(self.test), columns=self.variables
            )
    ...
    def outlier_detection(
        ...
        if strategy == "inter_quartile_range":
            ...
            forest_model.fit(self.train.loc[:, feature].values.reshape(-1, 1))
            pred_value_train = forest_model.predict(
                self.train.loc[:, feature].values.reshape(-1, 1)
            )
            pred_value_test = forest_model.predict(
                self.test.loc[:, feature].values.reshape(-1, 1)
            )

            train_out = np.where(pred_value_train == -1)[0].tolist()
            test_out = np.where(pred_value_test == -1)[0].tolist()

            train_outliers = self.train.iloc[train_out, :]
            test_outliers = self.test.iloc[test_out, :]
